...
{
    class SpriteAtlasTextureSpaceUsedIssueSettings : VisualElement
    {
        const string k_Uxml = "Packages/com.unity.2d.sprite-atlas-analyzer/Editor/Insider/SpriteAtlas/SpriteAtlasIssueReport/SpriteAtlasTextureSpaceUsedIssue/SpriteAtlasTextureSpaceUsedIssueSettings.uxml";

        IntegerField m_PageCountField;
        event Action<int> m_OnMemorySizeChanged;
        int m_PageCount;
        OKCancelElement m_OkCancelElement;
        public SpriteAtlasTextureSpaceUsedIssueSettings(int pageCount)
        {
            ...
            m_PageCountField = this.Q<IntegerField>("size");
        ...
        public event Action<int> onMemorySizeChanged
        {
            add => m_OnMemorySizeChanged += value;
            remove => m_OnMemorySizeChanged -= value;
        }

        void OnOKClicked()
        {
            m_PageCount = m_PageCountField.value;
            m_OnMemorySizeChanged?.Invoke(m_PageCount);
            m_OkCancelElement.EnableOKButton(false);
        }
    }
}
